...
（连续写成 64 hex 字符：`66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0`）
...
```
1F403...  # TODO: 填写确认后的测试值（请运行 tests/test_vectors.c）
```

> 请运行 `make test` 自动生成并核对，确保实现正确。
...
- 编写 AVX2 / NEON 优化版本；
- 完成长度扩展攻击脚本自动化；
- 构建 100k 叶子 Merkle 树并测性能；
- 撰写实验报告（可复用本 README 部分内容）。

---

如需我将该 README 与初始工程骨架（目录、占位代码）一并打包，请告诉我。
